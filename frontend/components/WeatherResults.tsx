  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex gap-2">
            <button
              onClick={handleShare}
              className="p-2 text-white hover:text-gray-700 transition-colors"
              title="Share results"
            >
        ...
        {/* Parade Score (single number) */}
        {typeof weatherData.paradeScore === 'number' && (
          <div className="flex items-center justify-between bg-white/10 rounded-lg p-4 mb-4">
            <div>
              <div className="text-xs text-white/80">Parade Score</div>
              <div className="text-4xl font-extrabold text-white">
                {weatherData.paradeScore}
                <span className="text-sm font-semibold text-white/70">/100</span>
              </div>
              {weatherData.paradeLabel && (
                <div className="text-xs mt-1 px-2 py-1 inline-block rounded-full bg-white/20 text-white">
                  {weatherData.paradeLabel}
                </div>
              )}
            </div>
            <div className="text-xs text-white/70 max-w-xs">
              Higher is better for outdoor plans. Score combines heat, rain, wind, and air quality.
            </div>
          </div>
        )}
      ...
      <div className="card bg-gray-50">
        <h3 className="text-sm font-semibold text-white mb-2">Data Source Information</h3>
